  ...
  rl.on("line", async (rawLine: string) => {
    ...
    if (line === "/help") {
      console.log([
        ...
        "  /exit    — 退出",
        "",
        "Change flow (default: structured preview first, then y/n/cancel):",
        '  "从现在起都排除 demo 标签"    → structured preview → confirm → persistent change',
        '  "从现在起相关度门槛改为 0.5"  → structured preview → confirm → persistent change',
        '  "从现在起只关注保险方向"      → structured preview → confirm → persistent change',
        "",
        "Session patch (compatibility — applies to this session only):",
        '  "这轮只看保险方向"            → immediate apply, focus for this session only',
        '  "降低相关度门槛到 0.2"        → immediate apply, threshold for this session only',
